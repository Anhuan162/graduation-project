...
import com.graduation.project.auth.dto.request.RefreshTokenRequest;
...
public class AuthController {
  ...
  @Operation(summary = "Đăng nhập hệ thống", description = """
      ...
      """)
  @ApiResponses({
      @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Đăng nhập thành công", content = @Content(schema = @Schema(implementation = AuthenticationResponse.class))),
  ...
  @io.swagger.v3.oas.annotations.parameters.RequestBody(required = true, description = "Thông tin đăng nhập của người dùng (email và password)", content = @Content(schema = @Schema(implementation = AuthenticationRequest.class), examples = {
      @ExampleObject(name = "Ví dụ đăng nhập", value = """
          {
            "email": "admin",
            "password": "123456"
          }
  ...
  @Operation(summary = "Làm mới access token", description = """
      ...
      """)
  @ApiResponses({
      @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Làm mới token thành công", content = @Content(schema = @Schema(implementation = RefreshTokenResponse.class))),
  ...
  public ApiResponse<RefreshTokenResponse> refresh(
      ...
      @RequestBody(required = false) RefreshTokenRequest bodyRequest) throws ParseException, JOSEException {

    String token = null;

    if (bodyRequest != null && bodyRequest.getRefreshToken() != null && !bodyRequest.getRefreshToken().isEmpty()) {
      token = bodyRequest.getRefreshToken();
    }
  ...
  @Operation(summary = "Kiểm tra tính hợp lệ của token (Introspect)", description = """
      ...
      """)
  @ApiResponses({
      @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Trả về trạng thái token", content = @Content(schema = @Schema(implementation = IntrospectResponse.class))),
      @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Token không hợp lệ hoặc định dạng sai")
  })
