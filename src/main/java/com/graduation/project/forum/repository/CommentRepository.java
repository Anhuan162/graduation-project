...
public interface CommentRepository extends JpaRepository<Comment, UUID> {
        @Query("SELECT new com.graduation.project.forum.dto.CommentWithReplyCountResponse("
                        + "c.id, c.content, c.author.id, c.author.fullName, c.author.avatarUrl, c.author.email, c.createdDateTime, COUNT(r), f.url, c.parent.id, c.reactionCount) "
                        + "FROM Comment c "
                        ...
                        + "GROUP BY c.id, c.content, c.author.id, c.author.fullName, c.author.avatarUrl, c.author.email, c.createdDateTime, f.url, c.parent.id, c.reactionCount")
