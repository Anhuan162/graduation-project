...
public interface PostRepository extends JpaRepository<Post, UUID> {
    ...
    @Query("SELECT p FROM Post p WHERE p.topic.id = :topicId AND p.deleted = false AND "
    ...
    @Query("UPDATE Post p SET p.reactionCount = COALESCE(p.reactionCount, 0) + 1 WHERE p.id = :postId")
    void increaseReactionCount(UUID postId);

    @Modifying(clearAutomatically = true)
    @Query("UPDATE Post p SET p.reactionCount = CASE WHEN COALESCE(p.reactionCount, 0) > 0 THEN p.reactionCount - 1 ELSE 0 END WHERE p.id = :postId")
    ...
    Page<Post> findAllByAuthor_IdAndPostStatus(UUID authorId, PostStatus postStatus, Pageable pageable);

    long countByAuthor_IdAndPostStatus(UUID authorId, PostStatus postStatus);
    ...
    @Query("SELECT new com.graduation.project.forum.dto.PostStatDTO(FUNCTION('to_char', p.createdDateTime, 'YYYY-MM-DD'), COUNT(p)) "
            + "FROM Post p "
    ...
    @Query("SELECT new com.graduation.project.forum.dto.PostStatDTO(FUNCTION('to_char', p.createdDateTime, 'YYYY-MM'), COUNT(p)) "
            + "FROM Post p "
            + "WHERE p.createdDateTime >= :startDate "
