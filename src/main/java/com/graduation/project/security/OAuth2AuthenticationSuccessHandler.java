...
public class OAuth2AuthenticationSuccessHandler implements AuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(
    ...
    String providerUserId =
        (String) oauthUser.getAttribute("sub"); // google uses "sub"; facebook uses "id"
    ...
    Optional<OauthAccount> accOpt =
        oauthRepo.findByProviderAndProviderUserId(Provider.valueOf(provider), providerUserId);
    User user;
    if (accOpt.isPresent()) {
    ...
    String target =
        String.format("%s?accessToken=%s&refreshToken=%s", redirectUri, accessToken, refreshToken);
